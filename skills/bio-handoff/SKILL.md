...
## 审批专用：fresh context + 只读口径合同（**硬门，非"最好"**）

⚠️ **认知前提**：**做过分析/整改的那条长上下文，去审自己产出，结构上就是被污染的**——不是"不够认真"，是 ①被"验收已通过"prime 成朝绿判、②orchestrator 审被审对象不独立、③长上下文本身退化(注意力稀释/中段遗忘，compact 存在即为此)。**这三点意志克服不了**，控制点不在 context 内部、在 context 的**边界**。所以终审规则是硬门：

**① 隔离**：**交付前终审必须在一个「没做过本项目分析/整改」的新会话里跑**（producer-context ≠ auditor-context）。同一人接手 → 先 `/clear` 或换会话，物理隔断 priming。这是硬要求，不是"最好"。
**② 只读集**：fresh 会话只加载「**审核口径合同段(HANDOFF) + spec 主轴 + 结果文件 + 数字台账(report_claims/numeric_reference)**」；**不加载整改叙事**（"Codex 修了 X、我验闭"那段）——"已全部收口"的叙事**缺席**正是要点。
**③ 主轴遍历**：审计**机械沿 spec 主轴逐条走**（`bio-result-auditor` 已内置），覆盖和主线的记忆/心情**解耦**；主轴遍历完 = 收敛边界（干净就 ship，防"再找找"无限回归）。
**④ 先独立判、后对账**：**先只读 spec+结果+台账形成自己的判定，再去 diff 上一轮"已闭/pass"**，分歧当信号（移动靶纪律）。**先读"已经收口了"就被锚死**——曾栽在让"整改已验闭"当先验。
**⑤ 对抗框架**：审计 prompt 问**"还缺什么/哪没做到"**(开放)，不是**"修复过了吗"**(朝绿)；承重结论**默认证伪**。

- 与 `bio-result-audit` / `bio-audit-fanout` 一条线：审批 = "fresh 子代理按合同 + spec 主轴复核"。
- **诚实边界**：这套修 **priming 偏差 + 已知 spec 覆盖确定性**，**不保证抓到 spec 都没预见的新问题**（那靠 off-spec 第二遍，尽力非保证）。别 over-claim 成"从此不漏"。
