...
- **过程记录不进交付包**：`HANDOFF.md`、`DOCS_INDEX.md`、`execution_log.md`、`fix_log.md`、`audit/`、`.work/`、`_archive/`、`proof.json`、`goal_proof.md`、`.bio_harness/`、各类 `*_v*` / `*_final` / 草稿 md——这些是内部记录/QA 证明，客户包只放正式报告/图/表/脚本/溯源表（proof 含 harness 路径，绝不能发客户）。源目录散落多版本 md 太多 → 先用 `bio-docs-tidy` 收口再打包。
