...
- Adds advisory/strict delivery gate hook. Keeps `skipDangerousModePermissionPrompt=true` (user's high-frequency outsourcing workflow); delivery safety comes from the gate + strict mode (`BIO_DELIVERY_GATE_STRICT=1` / `.bio_delivery_gate` / `delivery/.ready_to_send`), not from re-enabling per-action prompts.
