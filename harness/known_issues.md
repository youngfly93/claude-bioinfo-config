...
| 8 | 方法选择错误导致返工 | 方法有分歧/高风险/交付先走 `bio-grill` 分级对齐（协议单一真源在该 skill，此处不重述）；低风险任务列 3-5 点方法概要即可，别默认跳过对齐 | P2 |
| 9 | 会话止于规划不执行 | 低风险探索：plan 完即执行、不空等确认；高风险/交付/临床：走 `bio-grill` 分级对齐后再开跑（不无差别立即执行） | P2 |
